...
foreach ( $plugins as $plugin_data ) {
	...
	?>
	<tr <?php echo( true !== $plugin_data['License'] ? 'data-tooltip="' . esc_attr__( 'Unlicensed add-ons cannot be updated. Please purchase or renew a valid license.', 'give' ) . '"' : '' ); // @codingStandardsIgnoreLine ?>>
		<td><?php echo wp_kses( $plugin_name, wp_kses_allowed_html( 'post' ) ); ?></td>
		<td>
			<?php
			echo ( true === $plugin_data['License'] ) ? '<span class="dashicons dashicons-yes"></span>' . esc_html__( 'Licensed', 'give' ) : '<span class="dashicons dashicons-no-alt"></span>' . esc_html__( 'Unlicensed', 'give' ); // @codingStandardsIgnoreLine

			echo sprintf(
				' &ndash; %s &ndash; %s',
				sprintf( _x( 'by %s', 'by author', 'give' ), wp_kses( $author_name, wp_kses_allowed_html( 'post' ) ) ), // @codingStandardsIgnoreLine
				sprintf( __( '(Latest Version: %s)' ), esc_html( $plugin_data['update']->new_version ) )
			);
			?>
		</td>
	</tr>
